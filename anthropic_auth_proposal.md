...
This document explores how Janus users with a **Claude Code Max subscription** ($100-200/month) can leverage their subscription for Anthropic model access without paying for separate API credits. The key finding: **Anthropic explicitly prohibits third-party apps from using subscription authentication via their SDK**, but the **CLI-based approach Janus already implements is the officially supported workaround**.
...
|--------|---------------------|---------------|------------|-------|
| **Claude CLI Spawn** | Yes | Yes (via subprocess) | Low | Current Janus approach |
...
Janus can spawn this binary and capture output, effectively "borrowing" the subscription authentication.

---

## 3. Current Janus Implementation Analysis
...
These tools use the same pattern as Janus:
1. Check for CLI installation
2. Spawn `claude` with prompts
3. Parse JSON output

This validates that Janus's approach is the industry standard.
...
**Insight:** Could be useful if Janus ever integrates with Vercel AI SDK ecosystem.
...
2. Expose auth method in `janus doctor` output
...
1. **Janus's current approach is correct** - spawning the CLI is the officially supported way to use Max subscriptions programmatically
