...
cd C:\path\to\conclave_cli\python
...
```batch
@echo off
cd /d C:\path\to\conclave_cli\python
...
Set-Alias -Name cenv -Value Start-Conclave
```

Now type `cenv` to activate the Conclave environment.
...
- `C:\path\to\conclave_cli\python\venv\`
- `%APPDATA%\npm\`
